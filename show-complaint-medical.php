...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $db_select_complaint_stmt = $db_conn->prepare('SELECT `complaint_report`.* FROM `complaint_report` INNER JOIN `medical_report` on `complaint_report`.`id` = `medical_report`.`complaint_id` GROUP BY `complaint_report`.`id` ORDER BY `complaint_report`.`id`');
}
...
<section>
    <header>
        ...
        ?>
            <h1><a href="show-complaint-medical.php">Complaint Report #<?php print($request_id); ?></a></h1>
        <?php
        } else {
        ?>
            <h1>All Complaint Reports with Medical Reports</h1>
        <?php
        }
            ...
            ?>
                <tr>
                    ...
                    <td><?php foreach ($entering_employee as $element) {
                        ...
                        ?></td>
                    <td><a href="show-complaint?id=<?php echo $row['id']; ?>">show</a> <a href="edit-complaint.php?a=delete&id=<?php echo $row['id']; ?>">delete</a> <a href="edit-complaint.php?a=edit&id=<?php echo $row['id']; ?>">edit</a></td>
                </tr>
            <?php
